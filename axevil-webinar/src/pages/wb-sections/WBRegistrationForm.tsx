...
// Module-level guard so multiple WBRegistrationForm instances on the same page
// (e.g. the hero card + the bottom registration section) don't push duplicate
// `form_view` events into the GTM dataLayer when both come into view.
let formViewFired = false

interface WBRegistrationFormProps {
  /** Wrapper className for layout overrides (e.g. width caps, alignment). */
  className?: string
  /** Disclaimer alignment — 'center' for both section and hero card by default. */
  disclaimerAlign?: 'left' | 'center'
  /** Submit button visual: 'plain' = white pill (bottom section), 'cta' = WBCtaButton-style (hero card). */
  submitVariant?: 'plain' | 'cta'
}

export default function WBRegistrationForm({ className = '', disclaimerAlign = 'center', submitVariant = 'plain' }: WBRegistrationFormProps) {
        ...
        <fieldset disabled={isPending} className="m-0 p-0 border-0">
          ...
          {submitVariant === 'cta' ? (
            <button
              ...
              className="relative w-full flex items-center justify-center gap-2 h-14 md:h-16 px-6 sm:px-8 rounded-2xl font-inter-tight font-semibold text-[0.875rem] md:text-[1.125rem] text-btn-label transition-all duration-300 border-b-4 border-btn-border hover:shadow-[32px_32px_32px_rgba(255,255,255,0.25),12px_12px_16px_rgba(255,255,255,0.25),2px_2px_8px_rgba(255,255,255,0.5)] hover:scale-[1.02] focus-visible:outline focus-visible:outline-2 focus-visible:outline-white disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:shadow-none"
            ...
            >
              ...
              />
              <span
                className={`relative z-10 shrink-0 rounded-full ${isPending ? 'animate-pulse' : ''}`}
                style={{ width: 8, height: 8, background: 'var(--black-700)' }}
              />
              <span className="relative z-10">{isPending ? t.form.sending : t.form.submit}</span>
            </button>
          ) : (
            <button
              type="submit"
...
function FormModal({
  ...
  return createPortal(
    ...
    >
      ...
      <motion.div
        className={`relative w-full max-w-[30rem] rounded-2xl ${isSpeakerVariant ? 'p-8 sm:p-10' : 'p-6 sm:p-8'} overflow-hidden`}
        style={
          isSpeakerVariant
            ? {
            ...
            {actionHref && actionLabel && (
              isSpeakerVariant ? (
                <a
                  ...
                  className="relative self-center max-w-full flex items-center justify-center gap-2 min-h-14 md:min-h-16 px-6 sm:px-8 py-2 rounded-2xl font-inter-tight font-semibold text-[0.875rem] md:text-[1rem] text-btn-label transition-all duration-300 border-b-4 border-btn-border hover:shadow-[32px_32px_32px_rgba(255,255,255,0.25),12px_12px_16px_rgba(255,255,255,0.25),2px_2px_8px_rgba(255,255,255,0.5)] hover:scale-[1.02] focus-visible:outline focus-visible:outline-2 focus-visible:outline-white"
                >
                  <div className="absolute inset-0 rounded-2xl bg-white pointer-events-none" />
                  <img
                    alt=""
                  ...
                  <span
                    ...
                    style={{ width: 8, height: 8, background: 'var(--black-700)' }}
                  />
                  <span className="relative z-10 text-center leading-tight sm:whitespace-nowrap">{actionLabel}</span>
                </a>
              ) : (
                <a
                  href={actionHref}
