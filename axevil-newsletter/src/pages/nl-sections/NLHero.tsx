 ...
 *
 * Иллюстрация — тот же мокап почтового клиента, обрезанный по видимому содержимому
 * (альфа > 128). Обрезка именно по видимому, а не по любой ненулевой альфе: справа от
 * окна в исходнике идёт слабое свечение шириной 75 px, слева его нет, и при обрезке по
 * альфе > 6 окно вставало левее центра экрана на 11 px (Татьяна, 2026-08-21). Конверт внизу срезан краем самой
  ...
  return (
    <section className="relative w-full overflow-hidden" style={{ background: 'var(--black-100)', paddingTop: '72px' }}>
      ...
      <div className="lg:hidden relative w-full flex flex-col items-center overflow-hidden"
        // Отступ от шапки: было фиксированные 1.25rem, бейдж прилипал к меню
        // (Татьяна, 2026-08-21). Теперь он привязан к высоте экрана, потому что тут
        // сталкиваются два требования. Нужен воздух — и держится требование «первый
        // экран без прокрутки», с 23 июля. На обычном телефоне (844 svh) выходит 35 px
        // и форма кончается на 574 из 844; на 320×568 ужимается до 24 px, и форма
        // укладывается в 567 — фиксированные 40 px там вылезали за сгиб на 15 px.
        style={{ padding: 'clamp(1.25rem, 4.2svh, 2.5rem) 0 0' }}>
