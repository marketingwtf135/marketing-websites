 ...
 *
 * Картинка собрана не из скриншота кадра, а из двух экспортов узла: фон (3129:4504,
 * обрезанный по кадру) плюс иллюстрация (3129:4653), вставленная на своё место
 * 872×147 в кадре 1512×854. Сходится с рендером Figma до 0.45 из 255, то есть точно.
 *
 * Собирать пришлось потому, что в кадре поверх всего лежат два одинаковых градиента
 * затемнения (3129:4573 и 3129:4654, по 144 px у нижнего края). Запечённые в картинку,
 ...
 * нижней границе экрана. Поэтому градиенты вынесены в CSS и считаются от реальной высоты
 * первого экрана (см. слой затемнения в разметке).
 *
 * Экспорт иллюстрации приходит непрозрачным, с плоской подложкой #060606 шириной ~33 px
 * вокруг окна. В рендере Figma она видна так же, как была видна у нас: точечное полотно
 * обрывается прямоугольником по краям письма (Татьяна, 2026-08-21: «что за чёрные полоски
 * по краям письма? убрать»). Подложка снята заливкой от границы кадра — именно заливкой,
 * а не порогом по цвету: внутри окна есть пиксели такой же яркости, и глобальный порог
 * продырявил бы письмо. Заодно ушла светлая линия в 3 px по верхнему краю экспорта.
 *
 * Слой с прозрачностью нужен только мобильной вёрстке (hero-ill-m.webp); на десктопе он
 * вклеен в фон, потому что положение мокапа относительно полотна всё равно фиксировано
 * макетом, а один файл легче двух.
 *
 ...
 * окна. Считается от видимого содержимого, а не от рамки узла (872..1486 из 1512): вокруг
 * окна в экспорте лежала плоская подложка #060606 шириной ~33 px с каждой стороны, и она
 * снята — см. комментарий к сцене.
 */
const ILL_RIGHT = 97.09

/**
 * Левая граница видимого мокапа — доля от кадра. Рамка узла начинается с 57.67%, но там
 * ещё была тёмная подложка; после её снятия содержимое начинается ровно с 58.995%.
 * Сюда упирается текстовая колонка.
 */
const ILL_CONTENT_LEFT = 58.995
