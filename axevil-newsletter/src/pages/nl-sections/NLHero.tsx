 ...
 *
 * Иллюстрация непрозрачная, со своей тёмной подложкой — отдельным слоем поверх фона её
 * класть нельзя, подложка вырезала бы точки прямоугольником. В макете так и есть, и в
 * склейке получается ровно то же самое.
 *
...
/**
 * Правый край иллюстрации — доля от кадра (рамка узла 872..1486 из 1512). По нему
 * проверяем, что мокап не уехал за край окна.
 */
const ILL_RIGHT = 98.28

/**
 * Левая граница видимого мокапа — доля от кадра. Рамка узла начинается с 57.67%, но там
 * ещё тёмная подложка; замер по яркости композиции даёт резкий скачок ровно на 59.5%
 * (средняя яркость столбцов: 12.0 на 58–59%, 15.7 на 59–60%, 28.1 на 60–61%). Именно
 * сюда упирается текстовая колонка.
 */
const ILL_CONTENT_LEFT = 59.5
