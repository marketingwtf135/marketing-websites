...
export default function Block10News() {
  return (
    <section className="w-full bg-page-bg">
      <div className="mx-auto w-full max-w-content container-px pt-[60px] md:pt-[120px] pb-[60px] md:pb-[100px] flex flex-col gap-12 items-start">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between w-full gap-4">
          <div className="flex flex-col gap-6 items-start">
            ...
            <h2
              className="font-inter-tight font-semibold leading-none tracking-[-0.02em] text-transparent bg-clip-text pb-2"
              style={{ backgroundImage: 'linear-gradient(94deg, #A2A2A2 15.77%, #FFF 49.29%, #A2A2A2 82.81%)', fontSize: 'clamp(1.75rem, 4vw, 4rem)' }}
            >
              Market Intelligence
            </h2>
          </div>
          <button
            type="button"
            className="hidden sm:flex gap-2 h-12 items-center justify-center px-6 py-4 rounded-xl bg-white font-inter-tight font-semibold text-text-btn text-page-bg hover:scale-[1.02] transition-transform focus-visible:outline focus-visible:outline-2 focus-visible:outline-page-bg shrink-0"
          >
        ...
        {/* Cards grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-5 gap-y-4 items-start w-full">
          {NEWS.map((item) => (
            <a
              key={item.id}
              href="#"
              className="flex flex-col gap-6 items-start group focus-visible:outline focus-visible:outline-2 focus-visible:outline-white rounded-card w-full"
            >
              {/* Square image container */}
              <div className="border-[3px] border-border-subtle rounded-card w-full overflow-hidden relative h-[15rem] sm:h-auto sm:aspect-square">
                <img
                  alt={item.title}
                  src={item.img}
              ...
              {/* Copy */}
              <div className="flex flex-col gap-4 items-start px-2 text-white w-full">
                <p className="font-inter-tight font-medium text-h5 leading-[1.1] tracking-[-0.02em] w-full">{item.title}</p>
        ...
        <button
          type="button"
          className="sm:hidden flex gap-2 items-center justify-center px-6 rounded-[0.75rem] bg-white font-inter-tight font-semibold text-text-btn text-page-bg w-full focus-visible:outline focus-visible:outline-2 focus-visible:outline-page-bg"
          style={{ height: '3rem' }}
        >
          <img src="/img/block10/notebook-icon.svg" alt="" aria-hidden="true" width={20} height={20} />
          All articles
