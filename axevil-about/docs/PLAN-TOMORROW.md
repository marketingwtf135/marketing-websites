...
- [ ] Просмотр/редактирование/удаление отдельных пунктов очереди + ручная смена их viewport-метки.
- [ ] Тоггл превью на брейкпоинтах (Desktop/Tablet/Mobile) внутри preview-бокса.
- [ ] Перетаскиваемая/закрепляемая панель (drag + «pin»), чтобы не зависеть от позиции элемента.
