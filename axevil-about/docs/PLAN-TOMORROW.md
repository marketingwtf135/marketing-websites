...
**P2 — переносимость**
- [x] **Каталог компонентов авто-дискаверится из экспортов пакета** (`import * as DS`):
      новые DS-компоненты сами попадают в папку «Other»; курируемые папки — оверрайд-мапа.
      (Иконки пока списком — рантайм не читает `public/`, манифест неполный.)
- [ ] Вынести ds-agent в dev-export пакета: `@axevil/design-system/ds-agent`, подключение одной строкой
      во всех проектах. ⚠️ Требует републиша пакета (через «запушь дс»).
...
- [x] **Тоггл превью на брейкпоинтах** (Desktop/Tablet/Mobile) внутри preview-бокса —
      ширина + базовый font-size меняются, компонент реально рефлоу.
- [ ] Мультивыбор элементов → групповая правка. ⚠️ Тяжело: ломает текущую single-selection
      модель (sel/selElRef/reflow завязаны на один элемент) — нужен рефактор состояния.
- [ ] Эфемерный live-edit варианта на самом элементе (примерить до Save). ⚠️ Тяжело и хрупко:
      точное позиционирование рендера поверх живого узла + перенос его стилей. Preview-бокс
      с breakpoint-тогглом сейчас закрывает 80% потребности безопаснее.
- [ ] Скриншот выделения в правку. ⚠️ Тянет зависимость html2canvas (новый пакет).
